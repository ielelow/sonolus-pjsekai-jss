...
const circularEffectLayout = ({ w, h }) => {
    const l = -w;
    const r = w;
...
export const playLinearNoteEffect = (effect) => effect.spawn(linearEffectLayout(), 0.5, false);
export const playCircularNoteEffect = (effect) => effect.spawn(circularEffectLayout({ w: 1.75, h: 1.05 }), 0.6, false);
export const playDirectionalNoteEffect = (effect) => effect.spawn(linearEffectLayout(), 0.32, false);
export const playFlatNoteEffect = (effect) => effect.spawn(flatEffectLayout(), 0.6, false);
export const playLaneEffects = () => particle.effects.lane.spawn(perspectiveLayout({ l: -2, r: 2, b: lane.b, t: lane.t }), 0.3, false);
export const spawnCircularHoldEffect = () => particle.effects.normalSlideConnectorCircular.spawn(circularEffectLayout({ w: 3.5, h: 2.1 }), 1, true);
export const spawnLinearHoldEffect = () => particle.effects.normalSlideConnectorLinear.spawn(linearEffectLayout(), 1, true);
