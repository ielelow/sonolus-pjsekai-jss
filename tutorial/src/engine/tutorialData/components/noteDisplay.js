...
export const noteDisplay = {
    update() {
        ...
        if (mode === Mode.Overlay) {
            ...
            const b = 1 + note.h;
            if (useFallback) {
                skin.sprites.draw(ids.fallback, perspectiveLayout({ l, r, t, b }).mul(y), layer.note.body, 1);
            }
            else {
                skin.sprites.draw(ids.left, perspectiveLayout({ l, r: ml, t, b }).mul(y), layer.note.body, 1);
                skin.sprites.draw(ids.middle, perspectiveLayout({ l: ml, r: mr, t, b }).mul(y), layer.note.body, 1);
                skin.sprites.draw(ids.right, perspectiveLayout({ l: mr, r, t, b }).mul(y), layer.note.body, 1);
            }
        }
    },
    showOverlay(type) {
