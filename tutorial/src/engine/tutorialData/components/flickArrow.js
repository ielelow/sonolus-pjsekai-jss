    ...
    update() {
        ...
        if (mode === Mode.Overlay) {
            ...
            if (sprites.useFallback) {
                ...
                const t = 1 - h;
                const b = 1;
                sprites.fallback.draw(new Rect({ l, r, b, t }).translate(0, -h * s).mul(y), layer.note.arrow, 1 - Math.ease('In', 'Cubic', s));
            }
            else {
                const l = -1;
                const r = 1;
                ...
                const t = 1 - h;
                const b = 1;
                sprites.arrow.draw(new Rect({ l, r, b, t }).translate(0, -h * s).mul(y), layer.note.arrow, 1 - Math.ease('In', 'Cubic', s));
            }
        }
    },
    showOverlay() {
