...
export class FlickNote extends FlatNote {
    ...
    render() {
        ...
        if (options.markerAnimation) {
            ...
            skin.sprites.draw(this.arrow.sprite, this.arrow.layout.add(this.arrow.animation.mul(s)).mul(this.y), this.arrow.z, 1 - Math.ease('In', 'Cubic', s));
        }
        else {
    ...
    playDirectionalNoteEffect() {
        this.directionalEffect.spawn(linearEffectLayout({
            lane: this.import.lane,
            shear: this.flickImport.direction,
        }), 0.32, false);
    }
    isCorrectDirection(touch) {
        if (this.flickImport.direction === FlickDirection.Up)
            return true;
