...
export class SlideEndFlickNote extends FlickNote {
    ...
    touch() {
        if (time.now < this.inputTime.min)
            return;
        if (this.startInfo.state === EntityState.Active)
            return;
    ...
    earlyTouch() {
        if (this.startSharedMemory.lastActiveTime === time.now)
            return;
        const s = ease(this.slideImport.ease, Math.unlerpClamped(this.head.scaledTime, this.tail.scaledTime, timeScaleChanges.at(time.now - input.offset).scaledTime));
        ...
        for (const touch of touches) {
            if (touch.vr < minFlickVR)
                continue;
            if (!hitbox.contains(touch.lastPosition))
                continue;
            if (!touch.ended && hitbox.contains(touch.position))
                continue;
            this.complete(touch);
            return;
        }
