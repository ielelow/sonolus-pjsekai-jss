export class ClaimManager {
    claimed = levelMemory(Dictionary(16, Number, {
        index: Number,
        time: Number,
        hitbox: Rect,
        fullHitbox: Rect,
    ...
    getClaimedTouchIndex(index) {
        for (const [touchIndex, claimedStart] of this.claimed) {
            if (claimedStart.index === index)
                return touchIndex;
        }
        return -1;
    }
    ...
    findBestTouchIndex(time, hitbox, fullHitbox, checkTouch) {
        ...
        for (const touch of touches) {
            if (!checkTouch(touch))
                continue;
            if (!fullHitbox.contains(touch.position))
                continue;
            ...
            if (claimedStart.hitbox.contains(touch.position))
                continue;
