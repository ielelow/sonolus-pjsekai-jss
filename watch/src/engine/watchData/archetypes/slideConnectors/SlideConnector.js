...
export class SlideConnector extends Archetype {
  ...
  updateVisualType() {
    if (!replay.isReplay) {
      this.visual =
        time.now >= this.start.time ? VisualType.Activated : VisualType.Waiting;
      return;
    }
    ...
    this.visual =
      time.now >=
      this.start.time + this.slideStartNote.windows.good.max + input.offset
  ...
  renderConnector() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (this.useFallbackSprite) {
        ...
        const normalA =
          (Math.cos((time.now - this.start.time) * 2 * Math.PI) + 1) / 2;
