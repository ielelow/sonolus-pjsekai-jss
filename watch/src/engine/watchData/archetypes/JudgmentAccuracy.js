...
export class JudgmentAccuracy extends SpawnableArchetype({
    ...
    updateParallel() {
        ...
        if (this.spawnData.j != Judgment.Perfect && this.spawnData.j != Judgment.Miss) {
            ...
            else if (this.spawnData.fastLate == 1) skin.sprites.fast.draw(this.layout, this.z, a)
            else if (this.spawnData.fastLate == 2) skin.sprites.late.draw(this.layout, this.z, a)
        }
    }
    updateSequential() {
        if (this.spawnData.j != Judgment.Perfect && this.spawnData.j != Judgment.Miss)
