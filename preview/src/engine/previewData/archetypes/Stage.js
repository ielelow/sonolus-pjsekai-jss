...
export class Stage extends Archetype {
    ...
    renderPanels() {
        for (let i = 0; i < panel.count; i++) {
            ...
            const b = 0;
            const t = panel.h;
            skin.sprites.stageLeftBorder.draw(new Rect({
                l: x - 6.5,
                r: x - 6,
                b,
                t,
            }), layer.stage, 1);
            skin.sprites.stageRightBorder.draw(new Rect({
                l: x + 6,
                r: x + 6.5,
                b,
                t,
            ...
            for (let j = 0; j < 6; j++) {
                skin.sprites.lane.draw(new Rect({
                    l: x + (j - 3) * 2,
                    r: x + (j - 2) * 2,
                    b,
                    t,
    ...
    printMeasures() {
        if (!options.previewMeasure)
            return;
        for (let i = 4; i <= Math.floor(chart.beats); i += 4) {
            print(i / 4 + 1, bpmChanges.at(i).time, PrintFormat.MeasureCount, 0, PrintColor.Neutral, 'right');
        }
    }
}
