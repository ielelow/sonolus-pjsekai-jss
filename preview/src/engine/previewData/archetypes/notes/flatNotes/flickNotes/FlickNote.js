...
export class FlickNote extends FlatNote {
    ...
    render() {
        ...
        const arrowSpriteId = getArrowSpriteId(this.arrowSprites, this.import.size, this.flickImport.direction);
        if (skin.sprites.exists(arrowSpriteId)) {
            const w = (Math.clamp(this.import.size, 0, 3) * (-this.flickImport.direction || 1)) / 2;
            skin.sprites.draw(arrowSpriteId, new Rect({
            ...
            this.arrowSprites.fallback.draw(Quad.one
